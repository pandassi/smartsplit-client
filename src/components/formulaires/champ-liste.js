...
function required(value) {
  const result = value
    ? undefined
    : "Une sélection dans cette liste est obligatoire";
  return result;
}
...
export class ChampListeEntiteMusicaleAssistant extends Component {
  ...
  listeEntites() {
    ...
    axios
      .get(`http://api.smartsplit.org:8080/v1/entities`)
      .then(res => {
        let aOptions = [];

        res.data.forEach(elem => {
          if (
            !this.state.rightHolderId ||
            elem.members.includes(this.state.rightHolderId)
          ) {
            aOptions.push({
              key: elem.uuid,
              text: elem.name,
            ...
            {this.state.options && (
              <Wizard.Field
                ...
                componentProps={{
                  ...
                  label: this.state.etiquette,
                  placeholder: t(
                    "flot.split.collaborateur.attribut.etiquette.ajout"
                  ),
                  required: this.state.requis,
                  autoFocus: this.state.autoFocus,
                  fluid: true,
    ...
    axios
      ...
      .then(res => {
        ...
        let _options = res.data.map((elem, idx) => {
          let nom = `${elem.firstName || ""} ${elem.lastName || ""} ${
            elem.artistName ? `(${elem.artistName})` : ""
          }`;
        ...
        this.setState({ ayantDroit: _adParId }, () => {
          if (this.props.onRef) this.props.onRef(_adParId);
        });
          ...
          <div>
            {this.state.options && (
              <Wizard.Field
                ...
                name={this.state.modele}
                component={Form.Dropdown} // = dropdown
                validate={val => {
                ...
                componentProps={{
                  //tout ce qui est dans prps va se retrouver dans dropdown
                  //Comme dropdown dans champ-selection-multiple-ayant-droit
                  id: "collaborateur",
                  label: this.state.etiquette,
                  placeholder: t(
                    "flot.split.documente-ton-oeuvre.bouton.ajout"
                  ),
                  autoFocus: this.state.autoFocus,
                  fluid: true,
                  search: true,
                  ...
                  onSelect: e => {
                    let _nom = e.target.parentElement.getElementsByClassName(
                      "text"
                    )[0].innerText;
                    if (
                      this.props.fnSelect &&
                      this.state.nomsConnus.includes(_nom)
                    ) {
                      this.props.fnSelect();
                    }
                  },
