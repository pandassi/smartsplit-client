...
class ForgotPassword extends Component {
  ...
  validateEmail(value) {
    ...
    if (!value) {
      error = (
        <Translation>
          {t => <>{t("flot.split.inscription.email-requis")}</>}
        </Translation>
      );
      ...
      error = (
        <Translation>
          {t => <>{t("flot.split.inscription.email-invalide")}</>}
        </Translation>
      );
    }
    return error;
  ...
  forgotPasswordHandler = courriel => {
    axios
      .post(
        "http://api.smartsplit.org:8080/v1/rightHolders/emailToRightHolderId",
        {
          email: courriel
        }
      )
      .then(res => {
        ...
        axios
          .patch(
            `http://api.smartsplit.org:8080/v1/rightHolders/${rightHolderId}/requestSource`,
            {
